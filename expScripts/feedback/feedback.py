...
while len(TR)>1: #globalClock.getTime() <= (MR_settings['volumes'] * MR_settings['TR']) + 3:
    ...
    try:
        feedbackMsg = subjectService.subjectInterface.msgQueue.get(block=True, timeout=1) # from subjInterface.setResult(runNum, int(this_TR), B_prob)
    ...
    if len(keys):
        ...
        if states[0] == 'feedback':
            ...
            if morphParam<1:
                morphParam=1
            print("\n=============================================")
            print(f'Scanner_TR_ID={TR[0]},rtcloud_trID={trID},parameter={morphParam},B_prob={B_prob}')
